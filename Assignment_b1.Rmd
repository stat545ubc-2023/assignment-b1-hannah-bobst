...
# Tests
This section performs three equality tests for the function *max_values*. Tests are conducted using Examples 1, 2, and 3 (above) to determine if the output values are calculated correctly. Each test matches the function's output to the output of the manual calculation. 

The tests using Examples 1 and 2 ensure that the function works for different data sets (*penguins* and *gapminder*). The test using Example 3 ensures that the function works for different arguments. Rather than using multiple variables and groups (such as in Examples 1 and 2), Example 3's test uses only one variable and one group. 
